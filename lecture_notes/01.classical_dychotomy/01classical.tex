...
\begin{itemize}
\item The economy starts with an initial stock of $K_0$ chickens
\item Firms mix existing chicken $K$ with labor $L$, in order to produce more chicken $Y$
\item Newly produced chicken can be broiled and eaten $C$, or placed next to live chickens as investment $I$. $Y = C + I$ must hold
...
\item Policy: Central banks do not decide policy by fixing a money supply. They issue reserves to target nominal interest rates, and are willing to exchange reserves for cash one-to-one to accommodate changes in money demand.
...
\end{itemize}
Equilibrium definition: An equilibrium is a sequence of real variables $\{r_{t+1},C_t,K_{t+1}\}$, and nominal variables $\{i_{t+1},A_{t+1},P_t\}$ starting at $t=0$, with initial conditions $A_0,K_0$, and an exogenous process for $Y_t$, such that
\begin{itemize}
...
\begin{itemize}
\item Nominal holdings pinned down by $A_{t+1} = 0$
...
\item What is the Taylor Rule: {\color{red}{Hike interest rates aggressively when inflation occurs. Pins down inflation locally. Potential problems with inflationary explosions and the ZLB.}}
\end{itemize}
\end{frame}
