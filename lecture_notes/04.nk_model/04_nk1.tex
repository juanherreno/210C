...
\end{itemize}
\pause
\item The salt-water reply produced the ``New Keynesian'' model
\begin{itemize}
\item It spent the 1980s developing micro-foundations for price rigidities
\item Simple representative-agent rational-expectations models, but with frictions
...
\pause
\item It is not obvious how to do it. Alternatives:
\pause
\begin{itemize}
...
\item Firms are monopolistic competitors facing CES demand. You are experts.
\pause
...
\begin{align*}
C_t = \left(\int_0^1 C^{\frac{\theta-1}{\theta}}_{it} di \right)^{\frac{\theta}{\theta-1}} \text{     ,    } P_t = \left(\int_0^1 P^{1-\theta}_{it} di \right)^{\frac{1}{1-\theta}} 
\end{align*}
\end{itemize}
\end{frame}
...
\pause
\item We will log-linearize the model. Jung Hyun taught you how. Need to choose a linearization point. We will choose the equilibrium with flexible prices and zero inflation
\pause
